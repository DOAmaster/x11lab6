...
void init(void)
{
  ...
  //left box sphere
	o = &g.object[g.nobjects];
	o->type = TYPE_SPHERE;
	vecMake(-1700.0, -120.0, -1000.0, o->center);
	vecMake(0.0, 0.0, -1.0, o->norm);
	o->radius = 100.0;
	if (g.tilt) {
...
//	}
  ...
  //bottom box sphere
	o = &g.object[g.nobjects];
	o->type = TYPE_SPHERE;
	vecMake(0, -1700.0, -1000.0, o->center);
	vecMake(0.0, 0.0, -1.0, o->norm);
	o->radius = 100.0;
	if (g.tilt) {
		vecMake(0.0, 100.4, 1.0, o->norm);
...
bool getShadow(Vec p) {
  Vec v;
  vecMake(0.0, 1000.0, 0.0, g.lightPos);
  vecSub(g.lightPos, p , v);
  Flt distToLight = vecLength(v);
  vecNormalize(v);
  Ray ray;
  vecCopy(p, ray.o);
  vecCopy(v, ray.d);
...
for (i=0; i < g.nobjects; i++) {
  ...
  //set up checker board pattern
  //Object *o;
	o = &g.object[h];
	if (o->surface == SURF_CHECKER) {
		int x = 1000 + (closehit.p[0]/2) / 40.0;
		int y = 1000 + (closehit.p[2]/2) / 40.0;
		x = x % 2;
		y = y % 2;
		if (x==y) {
  ...
  if (o->specular) {
    Ray rr;
    //create ray from close it to reflect orgin
    vecCopy(closehit.p, rr.o);
    reflect(ray->d, closehit.norm, rr.d);
    //start color 0, new color will be given after trace
    Vec trgb = {0.0};
    //nudges ray origin
    rr.o[0] += rr.d[0] * 0.01;
    rr.o[1] += rr.d[1] * 0.01;
    rr.o[2] += rr.d[2] * 0.01;
    //recursive call for reflection rays
    //weight reduces down each time
    trace(&rr, trgb, weight*0.5);
    rgb[0] += trgb[0] * weight;
    rgb[1] += trgb[1] * weight;
    rgb[2] += trgb[2] * weight;
 ...
 // bool inShadow = getShadow(closehit.p);
  Flt dot = 0.0;
 // if (!inShadow) {
 ...
 // }
  ...
  //set color based off ambient light
